...
def generate_industry_charts(analyzer):
    ...
    if 'ngân hàng' in industry or 'bank' in industry:
        ...
        plt.tight_layout()
        charts.append({'title': f'{analyzer.symbol} - Phân tích Ngân hàng', 'image': plot_to_base64(fig)})
    ...
    # REAL ESTATE
        ...
        plt.tight_layout()
        charts.append({'title': f'{analyzer.symbol} - Phân tích Bất động sản', 'image': plot_to_base64(fig)})
    ...
    # INSURANCE
        ...
        plt.tight_layout()
        charts.append({'title': f'{analyzer.symbol} - Phân tích Bảo hiểm', 'image': plot_to_base64(fig)})
    ...
    # ENERGY (Oil & Gas, Power)
        ...
        plt.tight_layout()
        charts.append({'title': f'{analyzer.symbol} - Phân tích Năng lượng', 'image': plot_to_base64(fig)})
    
    # GENERAL (fallback for all other industries)
    else:
        try:
            # Try to find common financial columns
            ...
            for col in available_cols:
                if 'doanh thu' in col.lower() and 'tỷ' in col.lower():
                    rev_col = col
                    break
            ...
            margin_col = None
            for col in available_cols:
                if 'biên lợi nhuận' in col.lower() or 'biên ln' in col.lower() or ('margin' in col.lower()):
                    margin_col = col
                    break
            ...
            debt_col = None
            for col in available_cols:
                if 'nợ' in col.lower() and ('tỷ lệ' in col.lower() or '%' in col or 'lần' in col.lower()):
                    debt_col = col
                    break
            if not debt_col:
                for col in available_cols:
                    if 'đòn bẩy' in col.lower():
                        debt_col = col
                        break
            ...
            panel_configs = []
            if rev_col:
                panel_configs.append(('bar', rev_col, 'Doanh thu', 'Tỷ đồng', '#3b82f6'))
            if margin_col:
                panel_configs.append(('line', margin_col, 'Biên lợi nhuận', '%', '#10b981'))
            if roe_col:
                panel_configs.append(('line', roe_col, 'ROE', '%', '#f59e0b'))
            if debt_col:
                panel_configs.append(('line', debt_col, debt_col, '', '#ef4444'))
            if fcf_col:
                panel_configs.append(('bar_color', fcf_col, 'Dòng tiền tự do (FCF)', 'Tỷ đồng', '#8b5cf6'))
            if eps_col:
                panel_configs.append(('bar', eps_col, 'EPS', 'VNĐ', '#ec4899'))
            
            if panel_configs:
                n_panels = min(len(panel_configs), 4)
                rows = 2 if n_panels > 2 else 1
                cols = 2 if n_panels > 1 else 1
                fig, axes = plt.subplots(rows, cols, figsize=(14, 5 * rows))
                industry_name = analyzer.profile_info.get('industry', 'Doanh nghiệp')
                fig.suptitle(f'{analyzer.symbol} - PHÂN TÍCH {industry_name.upper()}', fontsize=14, fontweight='bold')
                ...
                for idx, (chart_type, col, title, ylabel, color) in enumerate(panel_configs[:n_panels]):
                    r, c = divmod(idx, cols)
                    ax = axes[r][c] if isinstance(axes[r], (list, np.ndarray)) else axes[r]
                    
                    if chart_type == 'bar':
                        ax.bar(range(len(plot_data)), plot_data[col], color=color, alpha=0.7)
                        ax.set_xticks(range(len(plot_data)))
                        ax.set_xticklabels(plot_data.index, rotation=45)
                    elif chart_type == 'bar_color':
                        bar_colors = ['#10b981' if x >= 0 else '#ef4444' for x in plot_data[col]]
                        ax.bar(range(len(plot_data)), plot_data[col], color=bar_colors, alpha=0.7)
                        ax.axhline(0, color='black', linewidth=1)
                        ax.set_xticks(range(len(plot_data)))
                        ax.set_xticklabels(plot_data.index, rotation=45)
                    else:
                        ax.plot(plot_data.index, plot_data[col], marker='o', color=color, linewidth=2)
                ...
                for idx in range(n_panels, total_slots):
                    r, c = divmod(idx, cols)
                    ax = axes[r][c] if isinstance(axes[r], (list, np.ndarray)) else axes[r]
                    ax.set_visible(False)
                
                plt.tight_layout()
                charts.append({'title': f'{analyzer.symbol} - Phân tích {industry_name}', 'image': plot_to_base64(fig)})
        except Exception as e:
            print(f"Error generating general industry chart: {e}")
    
    return charts

def generate_ownership_chart(analyzer):
    """Generate ownership pie chart with shareholder names and smart label placement"""
    ...
    try:
        ...
        # Build labels from shareholder names
        name_col = 'share_holder' if 'share_holder' in top_5.columns else None
        if name_col is None:
            for col in top_5.columns:
                if 'name' in col.lower() or 'holder' in col.lower() or 'ten' in col.lower():
                    name_col = col
                    break
        
        if name_col:
            labels = [str(n)[:30] for n in top_5[name_col].tolist()]
        else:
            labels = [f'Cổ đông {i+1}' for i in range(len(top_5))]
        if others > 0:
            labels.append('Khác')
        ...
        # Explode small slices slightly for visibility
        explode = [0.03 if s >= 5 else 0.08 for s in sizes]
        
        fig, ax = plt.subplots(figsize=(12, 8))
        
        # Custom autopct: only show percentage inside if slice is large enough
        ...
        # Add annotations for small slices (outside with leader lines)
        for i, (wedge, size) in enumerate(zip(wedges, sizes)):
            if size < 5 and size > 0:
                ...
                ax.annotate(
                    ...
                    fontsize=10,
                    fontweight='bold',
                    arrowprops=dict(arrowstyle='->', connectionstyle=connectionstyle, color='#666'),
                    color='#333'
                )
        
        # Legend with shareholder names and percentages
        ...
        ax.set_title(f'CƠ CẤU SỞ HỮU - {analyzer.symbol}', fontweight='bold', fontsize=14, pad=20)
        plt.tight_layout()
        return plot_to_base64(fig)
    except Exception as e:
        print(f"Error generating ownership chart: {e}")
        return None
...
def analyze():
    ...
    try:
        ...
        # --- AI PREDICTION (from app_simple.py) ---
        ...
        try:
            ...
            if revenue_col and net_income_col and equity_col:
                ...
                if len(quarterly_data) >= 2:
                    ...
                    if len(X_revenue) > 1 and len(y_revenue) > 1:
                        ...
                        # Calculate current quarter based on real date
                        from datetime import datetime
                        now = datetime.now()
                        current_year = now.year
                        current_q = (now.month - 1) // 3 + 1
                        current_quarter = f"{current_year}Q{current_q}"
                        
                        if current_q == 4:
                            next_quarter = f"{current_year + 1}Q1"
                        else:
                            next_quarter = f"{current_year}Q{current_q + 1}"
                        ...
                        if profit_change > 3:
                            recommendation_score += 1
                        if current_profit / current_revenue * 100 > 15 if current_revenue > 0 else False:
                            recommendation_score += 1
                        ...
                        if recommendation_score >= 3:
                            analysis_summary = "✅ Doanh nghiệp có triển vọng tích cực. Đánh giá: CÓ THỂ MUA."
                        ...
                        try:
                            fig_pred, axes_pred = plt.subplots(1, 3 if has_roe_model else 2, figsize=(14 if has_roe_model else 10, 5))
                            if not has_roe_model:
                                axes_pred = list(axes_pred)
                            
                            # Revenue chart
                            ax = axes_pred[0]
                            bars = ax.bar(['Hiện tại\n' + current_quarter, 'Dự đoán\n' + next_quarter], 
                                         [current_revenue, pred_revenue],
                                         color=['#3b82f6', '#8b5cf6'], width=0.5, edgecolor='white', linewidth=2)
                            ax.set_title('💰 Doanh Thu (Tỷ)', fontweight='bold', fontsize=12, pad=10)
                            ax.set_ylabel('Tỷ VNĐ', fontsize=10)
                            ...
                            bars = ax.bar(['Hiện tại\n' + current_quarter, 'Dự đoán\n' + next_quarter],
                                         ...
                                         color=['#10b981', '#f59e0b'], width=0.5, edgecolor='white', linewidth=2)
                            ax.set_title('📈 Lợi Nhuận Ròng (Tỷ)', fontweight='bold', fontsize=12, pad=10)
                            ax.set_ylabel('Tỷ VNĐ', fontsize=10)
                            ax.grid(True, alpha=0.3, axis='y', linestyle='--')
                            for bar in bars:
                                height = bar.get_height()
                            ...
                            # ROE chart (if available)
                            if has_roe_model:
                                ax = axes_pred[2]
                                bars = ax.bar(['Hiện tại\n' + current_quarter, 'Dự đoán\n' + next_quarter],
                                             [current_roe, pred_roe],
                                             color=['#ef4444', '#ec4899'], width=0.5, edgecolor='white', linewidth=2)
                                ax.set_title('📊 ROE (%)', fontweight='bold', fontsize=12, pad=10)
                            ...
                            fig_pred.suptitle(f'AI Dự Đoán Quý Tiếp Theo - {symbol}', fontweight='bold', fontsize=14, y=1.02)
                            plt.tight_layout()
                            prediction_result['prediction_chart'] = plot_to_base64(fig_pred)
                        except Exception as chart_err:
                            print(f"[AI PREDICT] Lỗi tạo biểu đồ dự đoán: {chart_err}")
                            prediction_result['prediction_chart'] = None
                        
                        print(f"[AI PREDICT] Dự đoán thành công cho {symbol}")
            
        except Exception as e:
            print(f"[AI PREDICT] Lỗi khi dự đoán AI cho {symbol}: {e}")
            import traceback
            traceback.print_exc()
